# Hi, I’m Nate 👋

I build software with intention — small features, clear logic, and real iteration.  
My background in systems analysis helps me turn blurry ideas into working solutions  
without losing structure, documentation, or sanity.

---

## 🚀 Flagship Projects  

These are my primary, actively developed projects — each one reflects real learning, iteration, and long-term potential.
| Repo | Description | Tech Stack | Preview |
|------|-------------|------------|---------|
| **Project: Vantage** <br> ![System](https://img.shields.io/badge/type-system-007EC6?style=flat-square) ![Observability](https://img.shields.io/badge/focus-observability-purple?style=flat-square) | **Spatial Observability Engine.** <br> Real-time 3D infrastructure visualization tool. Maps high-velocity distributed logs to a cyberpunk city topology using a decoupled producer/consumer architecture. | Python (FastAPI) · Kafka (Redpanda) · React Three Fiber · Docker | [![Vantage](https://github-readme-stats.vercel.app/api/pin/?username=NathanielCarballo&repo=Log-Metropolis)](https://github.com/NathanielCarballo/Log-Metropolis) |
| **RogueMon** <br> ![Game](https://img.shields.io/badge/type-game-blue?style=flat-square) ![Fullstack](https://img.shields.io/badge/scope-fullstack-success?style=flat-square) | Full-stack roguelike: battle flow, capture logic, state management, wave progression | React · Flask · Python · REST API | [![RogueMon](https://github-readme-stats.vercel.app/api/pin/?username=NathanielCarballo&repo=RogueMon)](https://github.com/NathanielCarballo/RogueMon) |
| **DiscordBot** <br> ![Bot](https://img.shields.io/badge/type-bot-purple?style=flat-square) ![Async](https://img.shields.io/badge/async-yes-informational?style=flat-square) | Async bot with external API calls & multi-command expansion | Python · Discord.py | [![DiscordBot](https://github-readme-stats.vercel.app/api/pin/?username=NathanielCarballo&repo=DiscordBot)](https://github.com/NathanielCarballo/DiscordBot) |
> _“These aren’t demos — they’re systems in motion.”_

---

## 🔧 Practice & Prototypes  

These repos support learning, experimentation, and iteration.  
They aren’t static — each one evolves as I do.
| Repo | Purpose | Preview |
|------|--------|---------|
| **IdleMiner** <br> ![prototype](https://img.shields.io/badge/type-prototype-lightgrey?style=flat-square) | Incremental game logic & progression system experiment | [![IdleMiner](https://github-readme-stats.vercel.app/api/pin/?username=NathanielCarballo&repo=IdleMiner)](https://github.com/NathanielCarballo/IdleMiner) |
| **LocalHost Games** <br> ![Studio](https://img.shields.io/badge/scope-game_studio-orange?style=flat-square) | Solo indie studio exploring cozy & retro game mechanics | 🚧 Coming Soon |
| **Dev Forge** <br> ![learning](https://img.shields.io/badge/type-practice-orange?style=flat-square) ![refactor](https://img.shields.io/badge/focus-refactoring-green?style=flat-square) | Core fundamentals, refactor cycles, growth log | [![dev-forge](https://github-readme-stats.vercel.app/api/pin/?username=NathanielCarballo&repo=dev-forge)](https://github.com/NathanielCarballo/dev-forge) |
---

## 🧠 Tech & Tools I Use

### Languages  

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![CSharp](https://img.shields.io/badge/C%23-239120?style=flat&logo=csharp&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-424242?style=flat&logo=database&logoColor=white)

### Frameworks & Libraries  

![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black)
![ThreeJS](https://img.shields.io/badge/Three.js-black?style=flat&logo=three.js&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![.NET Web API](https://img.shields.io/badge/.NET_Web_API-512BD4?style=flat&logo=dotnet&logoColor=white)

### Tools & Workflow  

![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Kafka](https://img.shields.io/badge/Redpanda_/_Kafka-CRIMS?style=flat&logo=apachekafka&logoColor=white&color=D12424)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![VSCode](https://img.shields.io/badge/VS%20Code-007ACC?style=flat&logo=visualstudiocode&logoColor=white)
![Obsidian](https://img.shields.io/badge/Obsidian-483699?style=flat&logo=obsidian&logoColor=white)

### Currently Learning  

![K8s](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![Async](https://img.shields.io/badge/Async_Programming-0288D1?style=flat&logo=python&logoColor=white)

---

  ![Nate's GitHub stats](https://github-readme-stats.vercel.app/api?username=NathanielCarballo&show_icons=true)  
  ![Top languages](https://github-readme-stats.vercel.app/api/top-langs/?username=NathanielCarballo&layout=compact)